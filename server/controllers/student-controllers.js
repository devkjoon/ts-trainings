...
const newStudent = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: 'Student created but email could not be sent', success: false });
  }
};
...
const updateStudent = async (req, res, next) => {
  ...
  try {
    ...
    student = await Student.findById(sid).populate('company', 'name');
...
const assignCourse = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: 'Course assigned successfully', assignment: newAssignment, success: true });
  } catch (err) {
        ...
        const progressData = await Promise.all(
          student.enrolledCourses.map(async (course) => {
            ...
            const completedModulesCount = modules.filter((module) =>
              student.completedModules.some(
                (completedModuleId) => String(completedModuleId) === String(module._id)
              )
            ).length;
...
const getStudentCourses = async (req, res, next) => {
  ...
  try {
    ...
    const enrolledCourses = student.enrolledCourses.filter(
      (course) =>
        !student.completedCourses.some(
          (completed) => completed.courseId.toString() === course._id.toString()
        )
    );
